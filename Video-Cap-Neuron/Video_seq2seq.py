...
class VideoCaptioning:
    ...
    def train_test_split(self,data,test_frac=0.2):
        # generate an equally spaced array with the same length as the data
        indices = np.arange(len(data))
        # shuffle the array
        np.random.shuffle(indices)
        # assign (1-test_frac)% of the data for training
        train_indices_rec = int((1 - test_frac)*len(data))
        # slice the shuffled indices up to the desired training portion
        indices_train = indices[:train_indices_rec]
        # allocate the remaining data to the test set
        indices_test = indices[train_indices_rec:]
        # use the slices to achieve train-test-split
